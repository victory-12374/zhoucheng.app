...
dirs_test=os.listdir(r'F:/Last_Test/test1')#读取原始数据所在的文件夹
input_file=[os.path.join(r'F:/Last_Test/test1',name) for name in dirs_test] #读取所有的原始数据
output_file=[os.path.join(r'F:/Last_Test/test2',name) for name in dirs_test] #经过预处理后的数据存放的位置
for i in range(0,len(input_file)):
    ...
    params['path'] = input_file[i]
    params['opath'] =output_file[i]
    argvs=sys.argv
    try:
